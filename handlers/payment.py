...
async def get_cart_crypto(inv_id: str):
    rec = await db_one(
        "SELECT * FROM cart_crypto_payments WHERE invoice_id=$1", (inv_id,)
    )
...
# ── Kaspi-корзина (таблица) ───────────────────────────
async def save_cart_kaspi(uid, amount, promo_code, discount, items: list, buyer_note: str = ""):
    try:
        return await db_insert(
            """INSERT INTO cart_kaspi_payments
               (user_id,amount,promo_code,discount,items,buyer_note,created_at)
               VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id""",
            (uid, amount, promo_code, discount, json.dumps(items), buyer_note,
...
async def get_cart_kaspi(kid: int):
    return await db_one(
        "SELECT * FROM cart_kaspi_payments WHERE id=$1", (kid,)
    )


async def set_cart_kaspi_status(kid: int, status: str, mgr_mid=None):
    if mgr_mid is not None:
        await db_run(
            "UPDATE cart_kaspi_payments SET status=$1, manager_msg_id=$2 WHERE id=$3",
            (status, mgr_mid, kid),
        )
    else:
        await db_run("UPDATE cart_kaspi_payments SET status=$1 WHERE id=$2", (status, kid))
